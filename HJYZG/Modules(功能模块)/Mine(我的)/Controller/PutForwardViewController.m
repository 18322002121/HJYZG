...
@interface PutForwardViewController() <UITableViewDelegate,UITableViewDataSource>
@property (nonatomic,strong) UITableView *tableView;
@property (nonatomic,strong) NSArray *dataArray;
@property (nonatomic,strong) NSMutableArray *modelArray;
@end
static NSString *const putForwardViewCell = @"PutForwardViewCell";
...
- (void)viewDidLoad {
    ...
    [self createTable];
    _dataArray = @[@{@"titleLabel":@"提现金额",@"inputField":@"最少100元起"},
                   @{@"titleLabel":@"银行卡号",@"inputField":@"请确保卡号无误"},
                   @{@"titleLabel":@"姓名",@"inputField":@"请输入与银行卡绑定的用户姓名"}
                   ];
}
...
-(void)createTable{
    ...
    [_tableView registerClass:[PutForwardViewCell class] forCellReuseIdentifier:putForwardViewCell];
...
{
    PutForwardViewCell *cell = [tableView dequeueReusableCellWithIdentifier:putForwardViewCell];
    self.modelArray = [NSMutableArray arrayWithCapacity:0];
    for (NSDictionary *dict in self.dataArray) {
        [self.modelArray addObject:[PutForwardModel initWithDictionary:dict]];
    }
    cell.putForwardModel = self.modelArray[indexPath.row];
    return cell;
}
