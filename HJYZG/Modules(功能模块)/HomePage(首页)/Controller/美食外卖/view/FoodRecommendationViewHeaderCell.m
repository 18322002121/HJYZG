//
//  FoodRecommendationViewHeaderCell.m
//  HJYZG
//
...
@interface FoodRecommendationViewHeaderCell ()
/** 商品图片 */
...
/** 精选规格 */
@property (nonatomic,strong) UIButton *specifications;
/** 合计*/
@property (nonatomic,strong) PublicLabel *totalPriceLabel;

@end
@implementation FoodRecommendationViewHeaderCell
...
//
        /** 销量 */
        _salesVolume = [PublicLabel labelWithFrame:CGRectMake(0, 0, 0, 0) text:@"月销量：6" textColor:[UIColor blackColor] font:[UIFont systemFontOfSize:13.0] textAlignment:NSTextAlignmentCenter backgroundColor:[UIColor clearColor]];
        [self addSubview:_salesVolume];

        _specifications = [PublicButton buttonWithFrame:CGRectMake(0, 0, 0, 0) buttonEdgeInsetsStyle:(UIButtonEdgeInsetsStyleRight) image:@"箭头_右" backgroundImageImage:@"" title:@"精选规格" titleColor:[UIColor whiteColor] textFont:[UIFont systemFontOfSize:13.0] backgroundColor:[UIColor grayColor] addView:self target:self action:@selector(specificationsClick:)];
        _specifications.layer.cornerRadius = 10;
        
        _totalPriceLabel = [PublicLabel labelWithFrame:CGRectMake(0, 0, 0, 0) text:@"合计：23元" textColor:[UIColor redColor] font:[UIFont systemFontOfSize:13.0] textAlignment:NSTextAlignmentCenter backgroundColor:[UIColor clearColor]];
...
//    }];
    ...
    [_salesVolume mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerY.mas_equalTo(self.distributionInstructions.mas_centerY).offset(0);
    }];
    
    [_totalPriceLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.mas_equalTo(self.specifications.mas_centerY).offset(0);
...
- (void)specificationsClick:(UIButton *)sender{
    !_specificationsButton ? : _specificationsButton(sender);
}
